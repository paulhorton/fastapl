...
* Compute fraction of sequences matching regex L$.
...
* Trim poly-A tail allowing for some miscalled bases.
...
* Sort records by sequence length.
`% fastapl5 -se 'len1 <=> len2'  proteins.fasta`

* Reformat records so that sequence line lengths are 100 (at most).
`% fastapl5 -pl 100  proteins.fasta`

* Print first 3 records.
`% fastapl5 -ge '$c++ < 3'  proteins.fasta`

* Select records matching given ids.
`% fastapl5 -b '%keep = qw(Q12154|GET3_YEAST 1 P24583|KPC1_YEAST 1 P38179|ALG3_YEAST 1)' -ge '$keep{$id}'  proteins.fasta`

* Select records matching an id in ids.txt, ids.txt holds one id per line.
`% fastapl5 -b '%keep = map {chop;$_,1} <STDIN>' -ge '$keep{$id}'  proteins.fasta  <  ids.txt`

* Reformat records so that record sequences are printed on one line
`% fastapl5 -p -1  proteins.fasta`

* Extract records with first field after ID matching /mito/.
`% fastapl5 -ge '$head[1] =~ /mito/'  proteins.fasta`

* Print forward and reverse complementary strands with appropriate change to ids.
`% fastapl5 -pe '$id .= "_Crick"; pr; rc; $id =~ s/Crick$/Watson/'  shortDNA.fasta`

* Convert DNA sense strand to RNA transcript sequence.
`% fastapl5 -pe '$seq =~ tr/tT/uU/'  shortDNA.fasta`

* Dump 6 possible frame translations of DNA sequence.
`% fastapl5 -e 'say $id;  say "  $_"  for translations'  DNAwithPolyAtail.fasta`

* Add comments listing ORF fragments of length 29 or more to DNA sequence records.
`% fastapl5 -pe '$comment .= sprintf "# %2d %s\n", @$_[2,3]  for ORFs 29'  DNAwithPolyAtail.fasta`

* Add comments listing ORFs of length 29 or more to DNA sequence records.
`% fastapl5 -pe '$comment .= sprintf "# %2d %s\n", @$_[2,3]  for ORFs "m29"'  DNAwithPolyAtail.fasta`

* Upcase sequences.
`% fastapl5 -pe '$seq = uc;'  DNAwithPolyAtail.fasta`

* Print 2-mer frequences for each seq.
`% fastapl5 -e 'say hstr kmers 2'  mitoGenomes.fa`

* Print 2-mer frequences for each seq in compact format.
`% fastapl5 -e 'say  "$id\t",  hstrf ": ", kmers 2'  mitoGenomes.fa`

* Print cumulative 2-mer frequences of sequences, one 2-mer per line.
`% fastapl5 -e 'addKmers %c, 2' -f 'say hstr %c'  mitoGenomes.fa`

* Print table of homopolymer runs of length 20 or more.
`% fastapl5 -e '$m = runs 20;  say  join "\t",  $a, $b, $b-$a, seq $a  while $m->ab'  yeast_chrII.fa`

* Color matches to 'CACA' on either strand red, also underline the matches on the reverse strand; show all of these on the forward strand.
-H -pe 'color red, matches "CACA=1"; ul matches "CACA_1"'  shortDNA.fasta

* Print sequences in format of 100 bases per line.
`% fastapl5 -pl 100  proteins.fasta`

* Count the number of records with first field 'E.R.'.
`% fastapl5 -ce '$head[1] eq "E.R."'  proteins.fasta`

* Print information encode in header as key=value pairs
`% fastapl5 -e 'say "  $id"; say hstr asHash'  proteinsHashHeaders.fasta`

* Print aromatic residues in bold with rule.
`% fastapl5 -r -pe 'bold matches "[YWF]"'  proteins.fasta`

* Print DNA as double stranded
`% fastapl5 -P  shortDNA.fasta`

* Colorize final four residues in xterm format.
`% fastapl5 -pe 'colorize -4,0'  proteins.fasta`

* Print sequence as double stranded with inline ruler and ORF colored ORF track in xterm format.
`% fastapl5 --ORF a110m80c  -Pr  -l100  InfluenzaA_H1N1_Iwate200903.fa`

* Print bases 100 bases starting at position 5000 (counting from zero) in each record with ruler track.
`% fastapl5 -Re 'say "$id 5000,5100"; ps 5000,5100'  mitoGenomes.fa`

* Print entries with residues colorized.
`% fastapl5 -C  proteins.fasta`

* Print entries with residues colorized as DNA.
`% fastapl5 -Cd  shortDNA.fasta`

* Dump final field of each head, treating header line as semicolon separated fields.
`% fastapl5 '-F;' -e 'say $head[-1]'  semicolonHeadFields.fasta`

* For circular genome, print the reverse strand sequence seqment of length 10 centered on the origin.
`% fastapl5 -Oe 'say seg 5, -5'  mitoGenomes.fa`

* Mark TATAT on forward strand red, allowing matches to overlap.
`% fastapl5 -pe 'color red, matches "TATAT%"'  m.fa`

* Mark matches to 'GAATAT' on either strand in inverse video, for circular DNA
`% fastapl5 -OPe 'iv matches "GAATAT="'  mitoGenomes.fa`

* Downcase $seq, then upcase matches to 'tata[at]a'.  Note the -d flag.
`% fastapl5 -dpe '$seq = lc; upcase matches "tatawa"'  yeast_chrII.fa`

* Print regex to case insensitively match an adenine followed by six bases coding for amino acids sequence 'MA'
`% fastapl5 -db 'say expandRegex "(?i)a<MA>"'`

* Count the number of start codons.  Works for RNA or DNA sequences, upper or lower case.
`% fastapl5 -e '$c=()=/(?i)(A[UT]G)/g; say "$id $c"'  yeast_chrII.fa`


#### fastqpl Examples
* Print records with id containing substring "4401B".
`fastqpl5 -g  -e  '$id =~ /4401B/'  longreads_original_sanger.fastq`

* For each record: print (id, p), where p is the probability that all bases are correct.
`fastqpl5 -e '$p = 1;  $p *= (1 - $_) for( errProb() ); print "$p\n"'  longreads_original_sanger.fastq`

* Extract fasta file from fastq input.
`fastqpl5 -e 'print ">$head\n$seq\n"'  longreads_original_sanger.fastq`

* Sort records in order of descending average Phred score.
`fastqpl5 -se '(sum Phred1) / length $seq1  <=>  (sum Phred2) / length $seq2'  longreads_original_sanger.fastq`

* Trim off first 4 bases of each read.
`fastqpl5 -pe 'trim 4'   longreads_original_sanger.fastq`

* Another way to trim off final 4 bases of each read.
`fastqpl5 -pe 'trim 0,-5'   longreads_original_sanger.fastq`

* Dump sequence with Phred scores.  Output looks like: 'FSRRS4401BE7HA  t:37 c:37 a:37 g:35 T:35 T:35...'
`fastqpl5 -e '@p = Phred; @v = pairwise {"$a:$b"} @seq, @p; say "$id  @v"'   longreads_original_sanger.fastq`

* Trim off any bases with error prob > 0.1 at end of read.  Discard record if none found.
`fastqpl5 -ge  'trim  0,  lastidx {$_ < 0.1} errProb;  $seq'   longreads_original_sanger.fastq`

* Add fake poly-A tail to each read.  Each added base having an increasing error prob.
...
* Keep the (shortest) prefix with the highest total adjusted (15 subtracted from each score) Phred scores.  Similar to BWA trimming.
`fastqpl5 -ge '$sum = 0;  ($i,$max) = maxidx apply {$_ = $sum += $_-15} Phred;  trim 0,$i;  $max > 0'  longreads_with_dummy.fastq`

* Keep the segment with the highest total adjusted (15 subtracted from each score) Phred scores.  Similar to Solexa trimming.
